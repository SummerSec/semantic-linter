...
- **目标默认按平台选择**：Codex 使用用户当前工作目录的 `AGENTS.md`；Claude Code 使用 `CLAUDE.md`。用户明确给出目标时按用户指定路径。
...
1. 用 `pwd` 取当前工作目录。
2. 选择目标：
   - 当前平台是 Codex，或用户提到 Codex / AGENTS.md：`<cwd>/AGENTS.md`
   - 当前平台是 Claude Code，或用户提到 Claude / CLAUDE.md：`<cwd>/CLAUDE.md`
3. 向用户复述完整目标路径，确认后再写。若用户想写到别的项目，用用户给定路径替代。
...
用定位到的脚本路径 `<SCRIPT>` 与目标 `<TARGET_MD>` 运行：

```bash
node "<SCRIPT>" "<TARGET_MD>"
```

输出 `Wrote semantic-rules.md and managed pointer into ...` 即成功（同时生成规则文件与目标文件指针）。
...
1. 说明已生成两份：`<cwd>/semantic-rules.md`（规则全文）与目标文件受管区指针。
